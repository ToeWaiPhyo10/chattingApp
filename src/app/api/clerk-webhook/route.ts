...
export async function POST(req: Request) {
  try {
    ...
    if (
      ...
      const userSubscription = user.privateMetadata.subscriptions || [];
      const updatedSubscription = userSubscription.filter(
        (subscription) => subscription.sessionId !== sessionId
      );
      await clerkClient.users.updateUser(user.id, {
        privateMetadata: { subscriptions: updatedSubscription },
      });
    }
